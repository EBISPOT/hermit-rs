// Corrected copy of tests/java/upstream/java/org/semanticweb/HermiT/tableau/
// BlockingValidatorTest.java (Java 37ec30ac), issues #32 and #33. Only the root
// nodes differ. Both ontologies use the inverse role INVR, and with inverse roles
// ValidatedSingleDirectBlockingChecker.canBeBlocker/canBeBlocked accept a tree
// node only when its parent is a tree or graph node. Upstream hangs a1, a2, b1,
// b2 and b3 directly off the NI roots a and b, so none of them can block or be
// blocked and both tests fail at their first blocking assertion in Java too.
// Here a (and b) become tree children of one fresh NI root r that has no labels
// or edges. They keep their own labels and edges and, as children of an NI node,
// still can neither block nor be blocked; every other node is now eligible, as
// the assertions intend. All assertions are unchanged; they pass in Java too.
...
public class BlockingValidatorTest extends AbstractReasonerInternalsTest {
    ...
    public void testOneInvalidBlock() {
        ...
        Node r=m_tableau.createNewNINode(emptySet);          // 0
        Node a=m_tableau.createNewTreeNode(emptySet,r);      // 1
        Node b=m_tableau.createNewTreeNode(emptySet,r);      // 2
